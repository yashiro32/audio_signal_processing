...
descriptorMapping = { 0: 'lowLevel.mfcc.mean.0',
                      ...
                      #13: 'lowlevel.mfcc.mean.13',
                      #14: 'lowlevel.mfcc.mean.14',
                      #15: 'lowlevel.mfcc.mean.15',
                      #16: 'lowlevel.mfcc.mean.16',
                      #17: 'lowlevel.mfcc.mean.17',
                      #18: 'lowlevel.mfcc.mean.18',
                      #19: 'lowlevel.mfcc.mean.19',
                      #20: 'lowlevel.mfcc.mean.20',
                      #21: 'lowlevel.mfcc.mean.21',
                      #22: 'lowlevel.mfcc.mean.22',
                      #23: 'lowlevel.mfcc.mean.23',
                      #24: 'lowlevel.mfcc.mean.24',
                      #25: 'lowlevel.mfcc.mean.25',
                      #26: 'lowlevel.mfcc.mean.26',
                      #27: 'lowlevel.mfcc.mean.27',
                      #28: 'lowlevel.mfcc.mean.28',
                      #29: 'lowlevel.mfcc.mean.29',
                      #30: 'lowlevel.mfcc.mean.30',
                      #31: 'lowlevel.mfcc.mean.31',
                      #32: 'lowlevel.mfcc.mean.32',
                      #33: 'lowlevel.mfcc.mean.33',
                      #34: 'lowlevel.mfcc.mean.34',
                      #35: 'lowlevel.mfcc.mean.35',
                      #36: 'lowlevel.mfcc.mean.36',
                      #37: 'lowlevel.mfcc.mean.37',
                      #38: 'lowlevel.mfcc.mean.38',
                      #39: 'lowlevel.mfcc.mean.39'
                      
                      #0: 'lowlevel.lpc.mean.0',
                      #1: 'lowlevel.lpc.mean.1',
                      #2: 'lowlevel.lpc.mean.2',
                      #3: 'lowlevel.lpc.mean.3',
                      #4: 'lowlevel.lpc.mean.4',
                      #5: 'lowlevel.lpc.mean.5',
                      #6: 'lowlevel.lpc.mean.6',
                      #7: 'lowlevel.lpc.mean.7',
                      #8: 'lowlevel.lpc.mean.8',
                      #9: 'lowlevel.lpc.mean.9',
                      #10: 'lowlevel.lpc.mean.10'
...
def descriptorPairScatterPlot(inputDir, descInput = (0,0), anotOn = 0):
  ...
  colors = ['r', 'g', 'c', 'b', 'k', 'm', 'y', '#30eaab', '#1a6666', '#9c7777']
  plt.figure()
  plt.hold(True)
  legArray = []
  catArray = []
...
def convFtrDict2List(ftrDict):
  ...
  for key in range(len(descriptorMapping.keys())):
    try:
      ...
      ### For Essentia JSON Files ###
      lname, dname, sname, ind = '.'.join(descriptorMapping[key].split('.')[:-3]), ''.join(descriptorMapping[key].split('.')[-3]), ''.join(descriptorMapping[key].split('.')[-2]), int(descriptorMapping[key].split('.')[-1])
      ftr.append(ftrDict[lname][dname][sname][ind])
    except:
      ### For FreeSound JSON Files ###
      ...
      ### For Essentia JSON Files ###
      try:
        lname, dname, sname = '.'.join(descriptorMapping[key].split('.')[:-2]), ''.join(descriptorMapping[key].split('.')[-2]), ''.join(descriptorMapping[key].split('.')[-1])
        ftr.append(ftrDict[lname][dname][sname])
      except:
        lname, dname, sname = '.'.join(descriptorMapping[key].split('.')[:-3]), ''.join(descriptorMapping[key].split('.')[-3]), ''.join(descriptorMapping[key].split('.')[-2])
        ftr.append(ftrDict[lname][dname][sname])
  ...
  for path, dname, fnames  in os.walk(inputDir):
    for fname in fnames:
      if descExt in fname.lower():
        ...
        ### For Essentia JSON Files ###
        #dict1 = {"lowlevel.mfcc.mean": [fDict['lowlevel']['mfcc']['mean']]}
        #dataDetails[cname][sname]={'file': fname, 'feature':dict1}
