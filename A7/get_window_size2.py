...
"""
def chirpTracker(inputFile='../../sounds/chirp-150-190-linear.wav', M=4000, H=1000):
    """
    Input:
    ...
    freqDevSlope = 0.02                         # slope increase of minimum frequency deviation
...
h = np.ceil(m / 4)
while True:
    M, H, tStamps, fTrackEst, fTrackTrue = chirpTracker(inputFile='../../sounds/speech-female.wav', M=m, H=h)
    tailF = 10 
    meanErr = np.mean(np.abs(fTrackTrue[tailF:-tailF,:] - fTrackEst[tailF:-tailF,:]),axis=0)  
    if meanErr[0] < 2.0 and meanErr[1] < 2.0:
        break
